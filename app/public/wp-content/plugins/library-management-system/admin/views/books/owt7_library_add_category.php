...
<div class="owt7-lms owt7-lms-books">
    ...
    <div class="owt7_library_add_category">
        ...
        <div class="page-container">
            ...
            <form class="owt7_lms_category_form" id="owt7_lms_category_form" action="javascript:void(0);" method="post" accept-charset="UTF-8">
                ...
                <div class="form-row">
                    ...
                    <div class="form-group">
                        <label for="owt7_txt_category_name"><?php _e("Name", "library-management-system"); ?><?php if ( LIBMNS_Admin_FREE::libmns_is_field_required( 'category', 'owt7_txt_category_name' ) ) { ?> <span class="required">*</span><?php } ?></label>
                        <input type="text"
                            <?php echo isset($params['action']) && $params['action'] == 'view' ? 'disabled' : ''; ?>
                            value="<?php echo isset($params['category']['name']) ? $params['category']['name'] : ''; ?>"
                            <?php if ( LIBMNS_Admin_FREE::libmns_is_field_required( 'category', 'owt7_txt_category_name' ) ) { ?>required <?php } ?>id="owt7_txt_category_name" name="owt7_txt_category_name" placeholder="<?php esc_attr_e( 'e.g. Science, History, Fiction', 'library-management-system' ); ?>">
                    </div>
                    <!-- Status -->
                    <div class="form-group">
                        <label for="owt7_dd_category_status"><?php _e("Status", "library-management-system"); ?><?php if ( LIBMNS_Admin_FREE::libmns_is_field_required( 'category', 'owt7_dd_category_status' ) ) { ?> <span class="required">*</span><?php } ?></label>
                        <select <?php echo isset($params['action']) && $params['action'] == 'view' ? 'disabled' : ''; ?>
                            id="owt7_dd_category_status" name="owt7_dd_category_status" <?php if ( LIBMNS_Admin_FREE::libmns_is_field_required( 'category', 'owt7_dd_category_status' ) ) { ?>required<?php } ?>>
                ...
                <?php if(isset($params['action']) && $params['action'] == 'view'){ }elseif( ( !isset($params['action']) || $params['action'] == 'add' ) && LIBMNS_Admin_FREE::libmns_current_user_can( 'owt7_lms_add_category' ) ){ ?>
                <div class="form-row buttons-group">
                    <button class="btn submit-save-btn" type="submit"><?php _e("Submit & Save", "library-management-system"); ?></button>
                </div>
                <?php } elseif( isset($params['action']) && $params['action'] == 'edit' && LIBMNS_Admin_FREE::libmns_current_user_can( 'owt7_lms_edit_category' ) ){ ?>
                <div class="form-row buttons-group">
                    <button class="btn submit-save-btn" type="submit"><?php _e("Submit & Save", "library-management-system"); ?></button>
                </div>
                <?php } ?>
